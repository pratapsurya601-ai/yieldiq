...
def store_bhavcopy(df: pd.DataFrame, db: Session) -> int:
    ...
    insert_df = pd.DataFrame({
        ...
        "volume": pd.to_numeric(df.get("volume", 0), errors="coerce").fillna(0).clip(upper=2**62).astype("int64"),
        "turnover_cr": df.get("turnover_cr").apply(_safe_float) if "turnover_cr" in df.columns else None,
        "delivery_qty": pd.to_numeric(df.get("delivery_qty", 0), errors="coerce").fillna(0).clip(upper=2**62).astype("int64") if "delivery_qty" in df.columns else 0,
