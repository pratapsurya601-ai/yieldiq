...
# Earnings-impact estimator (heuristic, public-read, no auth).
#   GET /api/v1/analysis/{ticker}/earnings-impact
# Bridges fresh quarterly result → rough sense of how the next
# nightly FV recompute may move. NOT a recompute; payload is
# labelled is_heuristic=true at every layer.
from backend.routers import earnings_impact as earnings_impact_router
app.include_router(earnings_impact_router.router)
# Forward earnings calendar + per-5%-beat FV-impact preview
# (heuristic, public-read, no auth).
#   GET /api/v1/public/earnings-calendar-fv-impact/{ticker}
# Pre-earnings positioning surface — bridges the next-scheduled
# earnings date + consensus EPS + this ticker's historical
# surprise track record to a per-5%-beat FV sensitivity. Uses the
# SAME sector multiplier × dampener as earnings_impact_service so
# the forward preview and the post-print reading agree on a beat
# day. is_heuristic=true at every layer; not a recompute.
from backend.routers import earnings_calendar_fv_impact as earnings_calendar_fv_impact_router
app.include_router(earnings_calendar_fv_impact_router.router)
# ELI15 thesis — 3-bullet plain-English explainer of the model's
# verdict, generated by Claude with mandatory SEBI post-filtering
# and a deterministic template fallback. Cached per (ticker, day).
