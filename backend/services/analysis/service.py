...
class AnalysisService(NarrativeMixin):
    ...
    def _get_full_analysis_inner(self, ticker: str) -> AnalysisResponse:
        ...
        # ── consensus_estimates DB fallback (fix/analyst-estimator-wiring) ─
        # Finnhub's free tier returns ZERO price-target coverage for
        # Indian `.NS` symbols, so the block above almost always lands on
        # None / coverage_count=0 for the NSE universe → the composite's
        # analyst estimator slot was perpetually empty and the FV
        # collapsed toward DCF-only. The real coverage IS in the DB:
        # scripts/refresh_consensus.py writes yfinance target_mean /
        # target_median / analyst_count (bare-ticker keyed) into
        # `consensus_estimates` nightly. Read it here when Finnhub came
        # back empty. Best-effort — the helper never raises; the worst
        # case stays the existing coverage_count=0 fallback.
        try:
            _needs_db_consensus = (
                _analyst_consensus is None
                or (_analyst_consensus.coverage_count or 0) <= 0
                or _analyst_consensus.price_target is None
                or (
                    _analyst_consensus.price_target.mean is None
                    and _analyst_consensus.price_target.median is None
                )
            )
            if _needs_db_consensus:
                from backend.services.consensus_estimates_service import (
                    get_consensus_for_ticker as _get_db_consensus,
                )
                _db_cons = _get_db_consensus(ticker)
                if _db_cons is not None:
                    _db_mean = _db_cons.get("target_mean")
                    _db_median = _db_cons.get("target_median")
                    _db_vs = None
                    try:
                        _db_basis = _db_median if _db_median else _db_mean
                        if price and price > 0 and _db_basis:
                            _db_vs = round(
                                (float(_db_basis) / float(price) - 1.0)
                                * 100.0,
                                1,
                            )
                    except Exception:
                    ...
                    _analyst_consensus = AnalystConsensus(
                        coverage_count=int(
                            _db_cons.get("analyst_count", 0) or 0
                        ),
                        ...
                        price_target=AnalystPriceTarget(
                            mean=_db_mean,
                            median=_db_median,
                            high=_db_cons.get("target_high"),
                            low=_db_cons.get("target_low"),
                            vs_current_pct=_db_vs,
                        ),
                        eps_estimate=None,
                        as_of=_db_cons.get("as_of"),
                        source=_db_cons.get("source") or "yfinance",
                    )
        except Exception as _db_exc:  # noqa: BLE001
            import logging as _logging
            _logging.getLogger("yieldiq.analysis").warning(
                "consensus_estimates fallback failed for %s: %s",
                ticker, _db_exc,
            )

        # Feed the resolved analyst target into `enriched` so the
        # composite (built below at the _cf_compute_composite call) can
        # consume it. The composite reads `enriched["analyst_consensus"]`
        # (dict, .price_target.mean/median) else
        # `enriched["wall_street_avg_target"]`. Both keys are ONLY read
        # there — nothing on the cold path sets them — so populating them
        # here (before the composite runs, after the response-only
        # `wall_street_avg_target` is sourced from raw["finnhub_price_target"])
        # is the wiring that finally lets the analyst estimator fire for
        # the .NS universe. Guard the whole step.
        try:
            if (
                _analyst_consensus is not None
                and _analyst_consensus.price_target is not None
                and isinstance(enriched, dict)
            ):
                _pt_obj = _analyst_consensus.price_target
                _analyst_avg_for_composite = _pt_obj.mean or _pt_obj.median
                if _analyst_avg_for_composite:
                    enriched["analyst_consensus"] = {
                        "coverage_count": _analyst_consensus.coverage_count,
                        "price_target": {
                            "mean": _pt_obj.mean,
                            "median": _pt_obj.median,
                        },
                        "source": _analyst_consensus.source,
                    }
                    enriched["wall_street_avg_target"] = float(
                        _analyst_avg_for_composite
                    )
        except Exception:
            pass
