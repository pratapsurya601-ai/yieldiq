...
CACHE_VERSION = 44  # fix/nbfc-wacc-floor: NBFC WACC floor (0.11) applied for `_NBFC_TICKERS` when CAPM output is below the floor. Fixes canary gate 4 for BAJFINANCE (was 0.098 from pure CAPM via models.forecaster.compute_wacc — BAJFINANCE routes through the P/B financial path so DCFEngine's NBFC premium at screener/dcf_engine.py:92-108 never instantiated). Floor-based classification by ticker set (not yfinance industry-label string matching) — reliable across data-source drift. Zero FV drift: floor affects surfaced `wacc` field only; fair value for financials is P/B-based and does not consume `wacc`. Bump invalidates v43 entries so redeploy surfaces the floored WACC immediately rather than serving stale 0.098 until TTL expiry. v43=moat floor "Moderate" band + strengths SSOT. v42=Day-3 W6+W9 rules. v41=PR-DET-2 + PR-D2 terminal_g clamp + NBFC WACC +50bps. v40=Day-3 sanity clamps. v39=Discover TTL. v38=PR-NTPC scenario order. v37=PR-BANKSC-2. v36=PR-BANKSC. v35=FV stability. v34=MoS formula. v33=scenarios. v32=MoS SoT.
