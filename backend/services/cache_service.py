...
CACHE_VERSION = 45  # fix/dividend-db-first: DividendService now reads from corporate_actions table (same source as GET /public/dividends/{ticker}) before falling through to yfinance .info. Root cause: TCS.NS was rendering "Dividends: None / No dividends paid" despite 20 NSE-recorded dividend payments in last 5 years (verified via /public/dividends/TCS.NS returning count:20, total_paid_5y:451.0). yfinance .info dropped lastDividendValue/dividendYield for TCS, the old early-return at dividend_service.py:74 skipped the .dividends series fallback entirely. New cascade: 1) corporate_actions DB (canonical for Indian tickers, TTM yield computed from series × current price), 2) yfinance .info (unchanged fallback for non-Indian tickers), 3) yfinance .dividends series fallback when .info fields blank but series has rows. Zero FV drift: fix affects InsightCards.dividend subtree only. Bump invalidates v44 entries so dividend-paying cached stocks recompute with their corporate_actions data surfaced. NOTE: PR #43 (fix/prism-pillars-completeness) also bumps to v45 independently — whichever merges second should bump to v46 with an extended comment covering both. v44=NBFC WACC floor. v43=moat floor Moderate band + strengths SSOT. v42=Day-3 W6+W9 rules. v41=PR-DET-2 + PR-D2 terminal_g clamp + NBFC WACC +50bps. v40=Day-3 sanity clamps. v39=Discover TTL. v38=PR-NTPC scenario order. v37=PR-BANKSC-2. v36=PR-BANKSC. v35=FV stability. v34=MoS formula. v33=scenarios. v32=MoS SoT.
