  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <nav className="text-xs text-caption mb-6 flex items-center gap-1.5">
        ...
        <span>/</span>
        <span className="text-caption font-medium">Reverse DCF</span>
      </nav>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div className="bg-bg dark:bg-surface rounded-xl p-4 border border-gray-100">
          <p className="text-[10px] text-caption uppercase tracking-wider mb-1">Current Price</p>
          <p className="text-lg font-bold text-ink font-mono">{fmt(data.current_price)}</p>
        </div>
        <div className="bg-bg dark:bg-surface rounded-xl p-4 border border-gray-100">
          <p className="text-[10px] text-caption uppercase tracking-wider mb-1">Historical Growth</p>
          <p className="text-lg font-bold text-ink">{pct(data.historical_growth)}</p>
        </div>
        <div className="bg-bg dark:bg-surface rounded-xl p-4 border border-gray-100">
          <p className="text-[10px] text-caption uppercase tracking-wider mb-1">FCF Yield</p>
          <p className="text-lg font-bold text-ink">{pct(data.fcf_yield, 2)}</p>
        </div>
        <div className="bg-bg dark:bg-surface rounded-xl p-4 border border-gray-100">
          <p className="text-[10px] text-caption uppercase tracking-wider mb-1">Price / FCF</p>
          <p className="text-lg font-bold text-ink">{data.price_to_fcf ? `${data.price_to_fcf.toFixed(1)}x` : "\u2014"}</p>
        </div>
      </div>
        ...
        <div className="space-y-5">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-mono font-bold text-ink">{pct(wacc, 1)}</span>
            </div>
            <input
              type="range"
              min={0.06}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm text-caption">Terminal Growth Rate</label>
              <span className="text-sm font-mono font-bold text-ink">{pct(terminalG, 1)}</span>
            </div>
            <input
              type="range"
              min={0.0}
      ...
      <div className="bg-bg dark:bg-surface border border-border rounded-2xl overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-bold text-ink">Growth Scenarios</h2>
          <p className="text-xs text-caption mt-1">What the stock is worth at different growth assumptions</p>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-bg dark:bg-surface border-b border-gray-100">
              <th className="text-left px-6 py-3 font-semibold text-caption">Scenario</th>
              <th className="text-right px-6 py-3 font-semibold text-caption">FCF Growth</th>
              <th className="text-right px-6 py-3 font-semibold text-caption">Implied IV</th>
              <th className="text-right px-6 py-3 font-semibold text-caption">MoS vs Price</th>
            </tr>
          </thead>
          <tbody>
            {sortedScenarios.map(([label, sc]) => (
              <tr key={label} className="border-b border-gray-100">
                ...
                <td className="px-6 py-3 text-right font-mono text-ink">{fmt(sc.implied_iv)}</td>
